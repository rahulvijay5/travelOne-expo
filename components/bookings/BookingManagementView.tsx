...
import { format, addDays, startOfDay, parseISO, eachDayOfInterval, subDays } from 'date-fns';
...
export default function BookingManagementView({ hotelId }: BookingManagementViewProps) {
      ...
      try {
        ...
        if (roomsData) {
          ...
          if (parsedData.hotelId === hotelId) {
            const roomsArray = Array.isArray(parsedData.rooms) ? parsedData.rooms : [];
  ...
  const renderDateNavigation = () => (
    <View className="flex-row items-center justify-between flex-grow">
      <Button onPress={() => handleDateChange(-7)} className="bg-gray-200 dark:bg-gray-700 p-2 rounded-full">
          ...
          <View>
            ...
            <View style={[styles.headerRow, isDark && styles.headerRowDark]}>
              <View style={[styles.roomHeader, isDark && styles.roomHeaderDark]}>
                <Text style={[styles.headerText, isDark && styles.headerTextDark]}>Room</Text>
              </View>
              {days.map((day) => (
                <View key={day.toISOString()} style={[styles.dayHeader, isDark && styles.dayHeaderDark]}>
                  <Text style={[styles.headerText, isDark && styles.headerTextDark]}>{format(day, 'EEE')}</Text>
                  <Text style={[styles.subHeaderText, isDark && styles.subHeaderTextDark]}>{format(day, 'MMM d')}</Text>
                </View>
              ))}
            </View>
            ...
            {/* Rooms Grid */}
            {rooms.map(room => (
              <View key={room.id} style={[styles.roomRow, isDark && styles.roomRowDark]}>
                <View style={[styles.roomCell, isDark && styles.roomCellDark]}>
                  <Text style={[styles.roomText, isDark && styles.roomTextDark]}>Room {room.roomNumber}</Text>
                </View>
                {days.map(day => {
