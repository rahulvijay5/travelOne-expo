    ...
    const initialize = async () => {
      try {
        ...
        if (userData.role === "USER") {
          setPreviousStays(
            HotelData.filter((item) => parseInt(item.hotelId) !== 1)
          );
        }
      } catch (error) {
      ...
      <View className="p-2 w-full">
        ...
        {(userRole === "OWNER" || userRole === "MANAGER") && (
          <>
            ...
            <Pressable
              className="flex-row items-center justify-start mb-4 py-4 pl-4 bg-lime-100 dark:bg-lime-950 rounded-xl"
        ...
        <Pressable
          ...
          onPress={() => {
            ...
            const message =
              "Hey, Check out this amazing app, for seemless checkIns!";
            const options = {
              title: "Share App",
              message: message,
              url: shareUrl,
        ...
        >
          <Feather name="arrow-up-right" size={24} color="#84cc16" />
          <Text className="ml-3 font-semibold dark:text-white text-lg">
            Share this app
          </Text>
        </Pressable>
