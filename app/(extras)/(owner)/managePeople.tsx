...
const ManagePeople = () => {
    ...
    try {
      ...
      if (response && response.length > 0) {
        ...
        const isAlreadyManager = managers.some(
          (manager) => manager.id === potentialManager.id
        );
        ...
        setSearchError(
          `No user exists with this phone number, type correct phone number or invite them to ${APP_NAME} first!`
        );
      }
    } catch (error) {
          ...
          onPress: async () => {
            try {
              ...
              if (error.message === "User is already a manager or owner") {
                setSearchError(
                  "This user is already a manager or owner of another hotel"
                );
  ...
  return (
    <View className="flex-1 p-4">
      {/* Search Section */}
      <View className="">
        <Text className="text-xl font-semibold mb-2 dark:text-white text-black">
          Search for Manager
        </Text>
        ...
        {searchResult ? (
          <View className="mt-4 p-4 rounded-lg border dark:border-gray-300 border-gray-800 flex-row justify-between items-center">
            <View className="">
              ...
              <Text className="text-muted-foreground  dark:text-white text-black">
                {searchResult.phoneNumber}
              </Text>
            </View>
            <Pressable
              onPress={handleAddManager}
              className=" p-2 dark:bg-blue-900 bg-blue-500 rounded-lg"
            >
      ...
      {/* Managers List */}
      <View>
        <Text className="text-xl font-semibold mb-4 dark:text-white text-black">
          Current Managers
        </Text>
        {managers.length === 0 ? (
          <Text className="text-muted-foreground dark:text-white text-black">
            ...
            {managers.map((manager) => (
              ...
              >
                <View>
                  ...
                  <Text className="text-muted-foreground text-sm dark:text-white text-black">
                    {manager.phoneNumber}
                  </Text>
                </View>
                <Pressable
